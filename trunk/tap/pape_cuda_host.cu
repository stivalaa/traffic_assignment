 ...
 *
 * $Id: pape_cuda_host.cu 703 2011-09-15 00:12:39Z astivala $
 *
 ...
 *    first_thru_node - first non-zone(start) node; <first_thru_node cannot be traversed
 ...
 *                        of predecessor link indices for each node
 *
 * Return value:
 *    None.
 * 
               ...
               double *distances, long *predecessors)
{
  ...
  cutilSafeCall( cudaMalloc((void **)&d_Ca, num_nodes*(1+num_start_nodes)*
                            sizeof(double)) );
  cutilSafeCall( cudaMalloc((void**)&d_Pa, num_nodes*(1+num_start_nodes)*
                            sizeof(long)) );
  ...
  cutilSafeCall( cudaMemcpy(distances, d_Ca, 
                            num_nodes*(1+num_start_nodes)*sizeof(double),
