 ...
 * Created: March 2011
 *
 * $Id: pape_kernels.cu 703 2011-09-15 00:12:39Z astivala $
 *
 ...
 *              Each prev[i,s] is link index from predecessor node
 *              of node i for startnode s
                                 ...
                                 double dist[], long prev[], long queue_next[])
{
  ...
  for (long v = blockIdx.x; v < num_nodes; v += gridDim.x) 
  {
    for (long i = threadIdx.x; i < num_start_nodes; i += blockDim.x)
    {
      long s = start_nodes[i];
#if defined(__DEVICE_EMULATION__) || (defined(DEBUG) && !defined(CUDA)) 
      fprintf(stderr, "pape_init_arrays node %d source %d\n", v, s);
#endif
 ...
 *                   must be intiizlize dto all INVALID except start_node=0
 *    prev  -  (OUT) predecessor link  array,
 *               must have space for num_nodes entries
 *              Each prev[i,s] is predecessor link index 
...
#if defined(__DEVICE_EMULATION__) || (defined(DEBUG) && !defined(CUDA)) 
  fprintf(stderr, "pape_kernel blockidx=%d threadidx=%d  idx = %ld, s = %ld\n", blockIdx.x,threadIdx.x, idx, s);
#endif
...
//          prev[v * num_start_nodes + s] = u;
          prev[v * num_start_nodes + s] = i; // NB this is link (edge) not node now
