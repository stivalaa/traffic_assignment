 ...
 *
 * $Id: tap_frankwolfe.c 435 2011-06-29 02:07:20Z astivala $
 *
 ...
 *              Each predlink[i] is link index of edge into node i
...
//    fprintf(stderr, "orig = %d\n", orig); //XXX
    sssp_pape_lll(Va, Ea, Wa, net->num_nodes+1, net->num_links, 
                       orig, net->first_thru_node, 
                       predlink, dist, queue_next);
...
int main(int argc, char *argv[])
{
  ...
  if (!warm_start_mode)
  {
    ...
    iter = 0;
    fw_assign(&net, demands, Va, Ea, link_costs, link_volumes, dist, queue_next, predlink);
