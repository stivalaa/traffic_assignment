...
/* modified by Alex Stivala to do (min,+) algebra instead of ordinary real (+,*) algebra */

/* ADS added these for dodgy min_identify unary function */
...
/* ADS added these hacks for spmv_ell below  to do (min,+) algebra*/
template<typename T>
  struct binary_min : public thrust::binary_function<T,T,T>
  {
      /* __host__ __device__*/   T operator()(const T &lhs, const T &rhs) const {return lhs < rhs ? lhs : rhs ;}
  }; 
template<typename T>
  struct min_identity : public thrust::unary_function<T,T>
  {
      /*   __host__ __device__*/ T operator()(const  T &x) const {return T(FLOATINF);} /* XXX FIXME FLOATINF*/
  };
...
//             thrust::multiplies<ValueType>(),
//             thrust::plus<ValueType>());
    // ADS changed to use (min,+) algebra
    spmv_ell(A, x, y,
             min_identity<ValueType>(),
             thrust::plus<ValueType>(),
             binary_min<ValueType>());
