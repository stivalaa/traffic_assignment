...
/* ADS added these for dodgy identity element for min hardcoded for double type */
                      ...
                      ValueType * y)
{
    ...
    {
       ValueType sum = FLOATINF;  // ADS identity for min is inf XXX FIXME hardocded for double
        ...
        for(IndexType n = 0; n < num_cols_per_row; n++)
        {
            ...
            if (col != invalid_index)
            {
                ...
                sum = fmin(sum, A_ij + fetch_x<UseCache>(col, x)); // ADS use (min,+) algebra FIXME harddcode to use double XXX
            }
