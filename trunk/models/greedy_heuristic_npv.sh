...
#
# This version uses NPV (subtracts upgrade costs) not just maximiznig Pv
#
...
#
# $Id: greedy_heuristic_npv.sh 742 2011-09-22 04:48:06Z astivala $
# 
###############################################################################
...
upgrade_set_and_cost=`mzn2fzn --output-to-stdout $MODELS_DIR/upgrade_subset_npv.mzn $FINALPERIOD_DZN_FILE | eclipse -e "flatzinc:fzn_run(fzn_ic)" | convert_eclipse_output_to_id_list.py -c $FINALPERIOD_DZN_FILE`
...
do 
    ...
    if [ $t -eq 0 ]; then
        flowoutput2dzn.py -t 0 -g $upgrade_set -p0  $MODS_FILE  $BASELINE_ORIG_FW_OUT $BASELINE_INDIVIDUAL_FW_OUT $BASELINE_PAIRWISE_FW_OUT $budget  > $PV_DZN_FILE
        scheduled_set_and_cost=`mzn2fzn --output-to-stdout $MODELS_DIR/upgrade_subset_npv.mzn $PV_DZN_FILE | eclipse -e "flatzinc:fzn_run(fzn_ic)" | convert_eclipse_output_to_id_list.py -c $PV_DZN_FILE`
